...
/**
 * compound 是一个AST的集合，它里面可能是多个AST的并列，例如：
 * 1. 多个function
 * 2. 函数主体中多个并列的语句
 * 2. 多个并列的参数
 * 
 * 因此遇到compound，需要遍历的方式去追溯它的list数据
*/
...
char* as_f_assignment(AST_T* ast)
{
    ...
    // 赋值语句的等号后面的AST被修改为了 AST_FUNCTION
    if(ast->value->type == AST_FUNCTION)
    {
        ...
        AST_T* ast_val = ast->value;

        char* as_val_val = as_f(ast_val->value);
...
/**
 * FUNCTION_AST 的value被初始化为COMPOUND
 * 如果遇到一个CALL AST，就需要考虑sp寄存器的读写
*/
...
char* as_f(AST_T* ast)
{
    ...
    switch(ast->type)
    {
        ...
        default: {printf("[As Frontend]: No frontend for AST of type `%s`\n", ast_type_to_str(ast->type)); exit(1);} break;
